...
The central design challenge: human time is the scarcest resource. A human speaks slowly, reads slowly, and can only hold so much in working memory at once. AI is infinitely parallelizable — add more instances, more GPUs, more API calls. You cannot add more hours to a human's day. Every design decision flows from this asymmetry.
...
| **Model** (in this system) | LLM (or another Model) + requirements + InputConfig |
| **RequiredAI model** | A Model exposed as an OpenAI-compatible endpoint; usable as the base for another Model |
...
Any development session can be replayed entirely — the file tree rebuilt from scratch by replaying the command stream. If you want to correct a decision made after a mistranscription, you correct it, then replay forward from that point.
...
def pertinent(context) -> bool:
    ...
    May inspect context.changed_files, context.event_stream, context.word_stream,
...
def validate(context) -> tuple[bool, str]:
    ...
    explanation is the revision prompt shown to the model when passed=False.
    May make LLM calls via context.ai_client.
    May inspect context.diff, context.file_contents, context.conversation, etc.
    """
    return True, ""
```
...
- Metadata lives in a Python `dataclass` instance (`meta`), not inline YAML. This is readable, type-safe, and IDE-friendly.
- Standard Python imports at the top, like any Python file.
- The `context` object is the interface to everything the requirement might need. It grows as needed.
...
The context is passed to both `pertinent` and `validate`. It contains at minimum:
...
Not all fields are always populated. `pertinent` should be cheap — it only needs what's necessary to decide relevance.
...
**Cannot:** Write files, commit, push, install packages, modify environment — hard requirements enforce this
...
This stage converts the drafting model's Q&A markdown output into an unambiguous, machine-executable assistant_interaction script. The script generation model knows the full assistant_interaction syntax intimately. It takes the draft ("the model says change lines 10-20 to this") and produces the actual structured commands ("AI_READ_LINES: /path:1:9", "AI_SAVE_START: /path", ...).

This stage exists because the Q&A-style output of the drafting model is deliberately ambiguous from a machine-parsing perspective — that ambiguity is what makes it good for drafting. Script generation resolves that ambiguity into a deterministic command sequence.
...
- Only say Yes, No, or provide a literal replacement block
